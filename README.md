...
```shell script
sender_point>send -f FILEPATH -b BANDWIDTH (optional)-l AMOUNT (optional)-n
```
...
and `TIMESTAMP_sender_send_encoded_output.csv`, files generated by sending a file without encoding named `TIMESTAMP_receiver_send_no_code_output.csv`<br>
and `TIMESTAMP_sender_send_no_code_output.csv`. All files have next header structure:
